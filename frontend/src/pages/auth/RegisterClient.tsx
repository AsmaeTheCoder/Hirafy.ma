...
import { Autocomplete, Button, Checkbox, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, FormControl, FormControlLabel, IconButton, InputAdornment, InputLabel, MenuItem, Select, Stack, TextField, Typography } from "@mui/material";
...
const RegisterClient = () => {
  ...
  // registre
    ...
    const [
      ...
      const {fname, lname, telephone, adresse, ville, email, password, confirmPassword} = formValue;
      ...
      const handleRegister = async (e: any) => {
        ...
        if (fname && lname && telephone && adresse && ville && email && password) {
          await registerUser({...formValue, role:'client'});
        }
      }
  ...
  return (
    <>
      <Helmet>
        <title>
          {t('titre.Sinscrireentantqueclient')}
        </title>
      </Helmet>
      <section style={{ width: "100%", marginBottom: "40px" }}>
          ...
          <form style={{ margin: "auto", padding: "15px", width: "650px", alignContent: "center" }} >
            <Stack spacing={4}>
              ...
              <ColorButton onClick={(e) => handleRegister(e)} variant="contained" sx={{ textDecoration: "none", textTransform: "none", borderRadius: "20px", fontWeight: "500", fontSize: "1.2rem" }} type="submit" fullWidth size='large' >
                {t('button.Sinscrire')}
              </ColorButton>
