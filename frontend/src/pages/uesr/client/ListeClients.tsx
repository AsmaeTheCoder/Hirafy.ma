...
import { useDeleteUserMutation, useDetailsUserQuery, useListeClientsMutation, useUpdateArtisanClientMutation } from '../../../services/userApi';
import { Button, TextField, Table, TableBody, TableContainer, TableHead, TableRow, Paper, TableCell, Box, tableCellClasses, Typography, Modal, InputAdornment, IconButton, Stack, Avatar } from '@mui/material';
...
const ListeClients = () => {
    ...
    const [listeClients, {data : clients}] = useListeClientsMutation();

    useEffect(() => {
        const fetchClientsData = async () => {
            await listeClients({ page, perPage });
        }
    ...
    const handleDelete = (id: any) => {
        confirm({ title: 'Confirmation', description: "Vous êtes sûr de supprimer ce client?",confirmationText: 'Oui', cancellationText: 'Non', confirmationButtonProps: { style: { color: "#E09540" } }, cancellationButtonProps: { style: { color: "#E09540" } } })
          .then( async () => {
            await deleteUser(id);
    ...
    const handleSubmit = async (e: any) => {
        ...
        if (!fname && !lname && !telephone && ! adresse && !ville && !email ) {
            ...
            await updateArtisanClient({...formValue, profil: profile, role: "client"});
  ...
  return (
    <>
        <Helmet>
            <title>
                {t('titre.Listeclients')}
            </title>
        </Helmet>

        <Stack spacing={4} sx={{ width: "100%", ml: "40px" }}>

            <h2>{t('titre.Listeclients')}</h2>
            ...
            <TableContainer component={Paper}>
                <Table sx={{ minWidth: 850, borderRadius: "20px" }} aria-label="simple table">
                    ...
                    <TableBody>
                        {clients && clients?.data.map((item: any, index: any)  => (
                            <TableRow key={index} sx={{ '&:last-child td, &:last-child th': { border: 0 } }} >
                                <TableCell align="center" component="th" scope="row">
                                    {/* <Box component="img" sx={{ width: "70px", height: "70px", borderRadius: "35px"}} src={item.profil} alt="green iguana" /> */}
            ...
            <Modal open={openEdit} onClose={handleCloseEdit} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description" >
                <Box sx={styleEdit}>
                    <Typography variant='h5' sx={{fontWeight: "700", fontSize: "1.8rem"}}>{t('titre.Client')}</Typography>
